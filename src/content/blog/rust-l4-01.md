...
之前在学习借用的是有有了解到，只能同时存在一个可变借用，和多个不可变借用。这个规则可能会导致某些功能难以实现
#### Cell
...
我们不能对一个不可变的值进行可变借用，这会破坏 Rust 的安全性保证，相反，你可以对一个可变值进行不可变借用。然而在某些场景中，可能会要求一个值可以在其方法内部被修改，同时对于其它代码不可变
